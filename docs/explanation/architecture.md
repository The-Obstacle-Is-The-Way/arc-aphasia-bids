...
> How bids-hub is designed and why.
...
The stable PyPI release has bugs with `Nifti.embed_storage`. We pin to GitHub main:
```toml
[tool.uv.sources]
datasets = { git = "https://github.com/huggingface/datasets.git", rev = "..." }
```
See [UPSTREAM_BUG.md](/UPSTREAM_BUG.md) for details.
